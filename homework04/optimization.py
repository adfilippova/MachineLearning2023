...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        def count_Armijo():
            ...
            while count_phi(alpha) > count_phi(0) + self.c1 * alpha * count_grad_phi(0):
                alpha = alpha / 2
            return alpha

        if self._method == 'Wolfe':
            alpha = scalar_search_wolfe2(phi=count_phi, derphi=count_grad_phi, c1=self.c1, c2=self.c2)[0]
            if alpha is None:
                alpha = count_Armijo()
